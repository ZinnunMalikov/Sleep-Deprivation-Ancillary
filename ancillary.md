...
- The test statistic of 235.663 with a df = 12 yields a p-value far below 0.05 (p = 1.32 * 10^-43) so we reject Ho. There is convincing evidence at the 5% alpha level that there is an association between sleep quality (1-5) and sleep time (hrs) in individuals like those surveyed. Note that since one of the twenty (5%) expected counts is under 5, we should be somewhat cautious of the results.
